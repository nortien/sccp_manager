...
<script>
    function DispayPhoneActionsKeyFormatter(value, row, index) {
        ...
        exp_dev += '<a href="config.php?display=extensions&amp;extdisplay=' + encodeURIComponent(row['name']) + '"><i class="fa fa-pencil"></i></a> &nbsp;';
        exp_dev += '<a class="clickable delete" data-id="' + sccpEscapeHtml(row['name']) + '"><i class="fa fa-trash"></i></a>';
        if (rmn_dev == 'yes') {
